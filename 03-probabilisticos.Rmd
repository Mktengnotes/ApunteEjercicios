...
2.  Sean $\theta_1$ y $\theta_2$ las probabilidades de registrar un
    ...
    a.  La probabilidad de observar exactamente $n$ eventos en el
        ...
        $$ LL \;=\; \sum_{i}\sum_{j}\sum_{k}\sum_{t} \ln\!\Big( \Pr(n_{jkt} \mid \theta_1,\theta_2,\pi) \Big) $$

    b.  Aplicamos la fórmula de bayes directamente:
        $$ \Pr(\lambda=\lambda_2 \mid n_{jkt}=17) = \frac{\Pr(n_{jkt}=17 \mid \lambda=\lambda_2)\,\Pr(\lambda=\lambda_2)} {\Pr(n_{jks}=17 \mid \lambda=\lambda_1)\,\Pr(\lambda=\lambda_1) + \Pr(n_{jkt}=17 \mid \lambda=\lambda_2)\,\Pr(\lambda=\lambda_2)}. $$
        Donde:
