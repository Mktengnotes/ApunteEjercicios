...
\hypersetup{
  pdftitle={Apunte Problemas Resueltos},
...
\title{Apunte Problemas Resueltos}
\author{Marcel Goic}
\date{2025-11-11}
...
\begin{itemize}
\item
  \emph{Modelos Probabilísticos}: En este, se asume que los agentes se comportan en base a decisiones aleatorias. Usualmente se utiliza cuando se tiene información reducida y/o agregada respecto al comportamiento de los agentes de estudio.
\item
  \emph{Enfoque Estructural}: En este, se asume que los agentes se comportan de manera racional, lo cual se traduce a que toman decisiones con el objetivo de maximizar sus utilidades (no necesariamente monetarias)
\end{itemize}

\hypertarget{formulas-utiles}{%
\chapter{Formulas utiles}\label{formulas-utiles}}
...
\chapter{Modelos de Regresion}\label{modelos-de-regresion}}
...
\item
  Definir el nivel de agregación de las variables apropiado para el
...
\item
  Elegir las variables explicativas a considerar para el modelo,
  realizar las transformaciones según lo solicitado por el problema de
  gestión.
\item
  Decidir si se incluirán interacciones entre las variables
  explicativas, como también si posee un efecto fijo (como también
  definir los índices de estos efectos).
\item
  Evaluar los modelos a partir de distintas métricas, ya sean
...
\item
  Interpretar los coeficientes más significativas de un modelo, para
  responder la pregunta de gestión original.
\end{enumerate}

\hypertarget{problemas-teuxf3ricos}{%
\section{Problemas Teóricos}\label{problemas-teuxf3ricos}}

\hypertarget{e1-signo-esperado-y-significancia}{%
\subsection{E1: Signo esperado y significancia}\label{e1-signo-esperado-y-significancia}}
...
\tightlist
\item
  Si el parámetro tiene el signo esperado y no es significativo:
  mantener.
\item
  Si el parámetro no tiene el signo esperado y no es significativo:
  eliminar.
\item
  Si el parámetro no tiene el signo esperado y es significativo:
...
\hypertarget{e2-regresiuxf3n-lineal-vs-de-poisson}{%
\subsection{E2: Regresión Lineal vs de Poisson}\label{e2-regresiuxf3n-lineal-vs-de-poisson}}
...
\hypertarget{e3-selecciuxf3n-automuxe1tica-de-variables}{%
\subsection{E3: Selección automática de variables}\label{e3-selecciuxf3n-automuxe1tica-de-variables}}
...
\hypertarget{e4-regresiuxf3n-log-nivel-y-log-log}{%
\subsection{E4: Regresión Log-Nivel y Log-Log}\label{e4-regresiuxf3n-log-nivel-y-log-log}}
...
\tightlist
\item
  Un aumento de una unidad en \(x_{1}\) causa un aumento de \(\beta_{0}\)
  unidades en \(y\).
\item
  Un aumento de una unidad en \(x_{2}\) causa un aumento de \(\beta_{2}\)\%
  unidades en \(y\).
\item
  Un aumento de un 1\% en \(x_{2}\) causa un aumento de \(\beta_{2}\)
  unidades en \(y\).
\item
  Un aumento de un 1\% en \(x_{2}\) causa un aumento de \(\beta_{2}\)\%
  unidades en \(y\).
...
\hypertarget{e5-capturar-efectos}{%
\subsection{E5: Capturar efectos}\label{e5-capturar-efectos}}
...
\tightlist
\item
  \(y=\beta_{0}+\sum_{i}\beta_{i}Dummy_{i},\quad i=\{\text{niños, jóvenes, adultos, ancianos}\}\)
\item
  \(y=\beta_{0}+\beta_{1}ln(edad)\)
\item
  \(y=\beta_{0}+\beta_{1}edad+\beta_{2}edad^{2}\)
\item
  \(y=\beta_{0}+\beta_{1}\exp(edad)\)
\item
  Ninguna de las anteriores.
\end{enumerate}

\hypertarget{e6-coeficientes-y-paruxe1metros}{%
\subsection{E6: Coeficientes y Parámetros}\label{e6-coeficientes-y-paruxe1metros}}
...
\tightlist
\item
  Transformar \(\gamma t\) en \(\gamma_{t}\) no influye en el modelo ya
...
\item
  Los parámetros \(\theta_{s}\) y \(\mu_{is}\) no pueden estimarse
  simultáneamente.
\end{enumerate}

\hypertarget{e7-quuxe9-es-un-modelo-de-regresiuxf3n}{%
\subsection{E7: ¿Qué es un modelo de Regresión?}\label{e7-quuxe9-es-un-modelo-de-regresiuxf3n}}
...
\item
  Dependiendo de la especificación puede determinar causalidad o
  correlación, pero no ambas simultáneamente.
\end{enumerate}

\hypertarget{soluciuxf3n-problemas-teuxf3ricos}{%
\subsection{Solución problemas teóricos}\label{soluciuxf3n-problemas-teuxf3ricos}}

\textbf{E1: Signo esperado y Significancia}\\
Respuesta correcta: e.\\
\{ Ninguna de las anteriores. Por ejemplo: no tiene signo esperado y no
es significativo: Mantener\}

\textbf{E2: Regresión Lineal vs de Poisson}\\
...
\def\labelenumi{\arabic{enumi}.}
\item
  Formule un modelo de regresión lineal que capture los siguientes
  factores:
  ...
  \tightlist
  \item
    Existe un efecto fijo por producto y por tienda, es decir, la
    linea base de las ventas es distinto para cada producto
    dependiendo de la tienda.
  \item
    Si un producto no se vende en una tienda en un día dado es
    probable que aya un quiebre de stock y si un día hay quiebre, es
    probable que el día siguiente el producto siga quebrado.
  \item
    El conjunto \(\mathcal{PN}\) registra todos los productos de la
    firma que son de primera necesidad. Las cuarentenass no tienen
    efecto en os productos de primera necesidad.
  \item
    El efecto de las cuarentenas puede ser distinto para las ventas
    de un día de semana con respecto a un fin de semana.
  \item
    El nivel de progresión de las infecciones (\(InfMV_{st}\)) afecta
    el nivel de ventas.
  \item
    El nivel de progresión de las infecciones afecta el nivel de
  ...
  \end{enumerate}
\item
  Considere los siguientes dos modelos, los cuales contienen variables
  ...
  \[
  \textbf{Tabla: } \text{Fragmento de parámetros y métricas de ajuste para dos modelos alternativos.} \\
...
\item
  Inspeccionando los resultados del modelo de regresión lineal que
...
\def\labelenumi{\arabic{enumi}.}
\item
  Agregamos progresivamente un término adicional a la regresión:
...
\item
  El modelo 1 es mejor que el modelo 2 en casi todas las métricas
  ...
  \(R_{adj.}^2\).
\item
  Para el modelo preferido (1) todos los coeficientes asociados a las
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 18\tabcolsep) * \real{0.10}}@{}}
\toprule
IdFunción\textsubscript{i} & Fecha\textsubscript{i} & Hora\textsubscript{i} & Película\textsubscript{i} & Duración\textsubscript{i} & Tipo\textsubscript{i} & Edad\textsubscript{i} & Estudio\textsubscript{i} & Presupuesto\textsubscript{i} & Ingreso\textsubscript{i} \\
\midrule
\endhead
...
\[
\textbf{Tabla: } \text{Ejemplo de 6 registro en Tabla de Ventas.}
\]

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Nótese que una película puede tener varias funciones y que, debido a
...
\item
  El equipo escogió como variable dependiente el logaritmo de la suma
  ...
  \[
  \textbf{Tabla: } \text{Parámetros y métricas de ajuste para dos modelos alternativos.}
  \]
...
\item
  Interprete los coeficientes de cada modelo. Según cada modelo, ¿qué
  tipo de películas debería presentar el cine para tener mayores
  ingresos?
\item
  Suponga que en su base de datos posee también la variable
  ...
  \def\labelenumii{\alph{enumii}.}
  \item
    Se debe considerar el efecto marginal por parte de las variables
    Presupuesto, Duración, Edad, Tipo, Estudio y \(NFun\).
  \item
    Existe un efecto en las ventas que depende simultáneamente del
  ...
  \item
    El éxito de la películas apropiadas para niños (Edad \(\in\) \{TE,
...
\item
  ...
  \begin{longtable}[]{@{}
    ...
    >{\centering\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.14}}@{}}
  \toprule
  \(Pelí  cula_j\) & \(Dura  ción_j\) & \$ Tipo\_j\$ & \$ Edad\_j\$ & \(Est  udio_j\) & \(log(  Pre_j)\) & \(log(  Ing_j)\) \\
  \midrule
  \endhead
  ...
  \end{longtable}
\item
  El modelo que explica de manera más efectiva el comportamiento es el
  ...
  \item
    ...
    coeficientes del modelo 2 tienden a ser \textless5\%
  \item
  ...
  \item
    Los valores de intercepto en el modelo 1 está demasiado
...
\item
  ...
  \item
    \textbf{Observación}: En el modelo 1, sería deseable considerar la
...
\item
  ...
  \def\labelenumii{\alph{enumii}.}
  \item
    La condición (a) muestra que se deben incluir las variables
    Presupuesto, Duración, Edad, Género, Estudio y Nfun en el
    modelo, con sus propios coeficientes.
  \item
    La condición (b) nos dice que se debe agregar además, las
    variables de estudio condicionada al género.
  \item
    Finalmente, la condición (c) dice que queremos conocer el
...
\[\text{Tabla: Variables Demográficas}\]
...
\[\text{Tabla: Variables Transaccionales}\]

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Como primera aproximación, se propone modelar la acreditación
  ...
  \tightlist
  \item
    ¿Cuál es la interpretación de los parámetros \(\alpha_{i}\),
    \(\alpha_{t}\), \(\gamma_{i}\) y \(\gamma_{t}\)?
  \item
    ¿Son los modelos anteriores identificables? En caso de ser su
    respuesta negativa, ¿cómo se corrige el problema?
  \end{enumerate}
\item
  Suponga que se han identificado los modelos anteriores (de
  ...
  de tiempo \(t\).
\item
  Proponga especificaciones de los modelos de regresión que permitan
  capturar que:
  ...
  \tightlist
  \item
    El impacto que tiene informar el estado de cuenta de los puntos
    depende del género.
  \item
    Los niveles de actividad (acreditación y canje) de un cliente
    varían significativamente de acuerdo a su categoría.
  \item
    El nivel de actividad (acreditación y canje) depende de la
    antiguedad de los clientes en el programa, pero aquellos que
    llevan más años tienden a tener un comportamiento similar.
  \end{enumerate}
\item
  Suponga ahora que se han estimado tres especificaciones para
  ...
  \[\text{Tabla: Resultados Modelos Regresión}\]

  \begin{enumerate}
  \def\labelenumii{\alph{enumii}.}
  \tightlist
  \item
    ¿Qué variables ayudan a explicar el comportamiento de
    acreditación de los consumidores?
  \item
    ¿Cuál es el medio en que se acreditan o acumulan, en promedio,
    menos puntos?
  \item
...
\item
  ...
  \tightlist
  \item
    Los parámetros \(\alpha_{i}\) y \(\gamma_{i}\) representan el efecto
    ...
    anteriores).
  \item
    Al estar las constantes \(\alpha\) y \(\gamma\), junto con los
  ...
  \end{enumerate}
\item
  Sabemos que \(\mathbb{E}(Y|X)=X'\hat{\beta}_{OLS}\). Luego, el balance
...
\item
  ...
  \tightlist
  \item
    Se debe incorporar una interacción entre las variables de \emph{Mail}
    y \emph{Género}.
  \item
    Se debe transformar la variable categórica en dummies y omitir
    uno de los niveles para no tener variables colineales.
  \item
    Se debe construir la variable logaritmo de antigüedad como
...
\item
  ...
  \tightlist
  \item
    Las variables que son significativas independiente del modelo en
    el que fueron estimadas, y que permiten explicar el
    comportamiento de interés, es decir, \emph{Ingreso}, \emph{Promoción}, y
    \emph{TiendasPropias}.
  \item
    El parámetro asociado al medio \emph{TarjetaBancaria} fue fijado en
    ...
    acreditan.
  \item
    La respuesta dependerá del criterio de evaluación. Si se quiere
...
\hypertarget{problema-4---p1-control-1-primavera-2025}{%
\subsection{Problema 4 - P1 Control 1 Primavera 2025}\label{problema-4---p1-control-1-primavera-2025}}
...
\tightlist
\item
  \textbf{ID:} Identificador de la propiedad.
\item
  \textbf{Ciudad}: ciudad.
\item
  \textbf{Pais}: país.
\item
  \textbf{Año}: Año en la que se regista el precio de la propiedad.
\item
  \textbf{PrecioUSD}: precio publicado de la propiedad en USD.
\item
  \(M^2\): superficie en metros cuadrados.
\item
  \textbf{Dorms}: número de dormitorios.
\item
  \textbf{Baths}: número de baños.
\item
  \textbf{Centro}: distancia (km) al centro de la ciudad.
\item
  \textbf{Residencial}: 1 si la propiedad es residencial (0 si es
  comercial).
\item
  \textbf{AñoConstrucción}: año de construcción de la propiedad.
\item
  \textbf{CiudadIPC}: ingreso per cápita promedio de la ciudad (USD PPP).
\item
  \textbf{CiudadPob}: Población total de la ciudad.
\item
  \textbf{CiudadSup}: Superficie total de la ciudad.
\end{itemize}
...
\def\labelenumi{\arabic{enumi}.}
\item
  (1.0 puntos) Formule un modelo de regresión para describir el
  ...
  \tightlist
  \item
    Existe un efecto fijo por país-año, que permite establecer si,
  ...
  \item
    La superficie, la antigüedad, el número de dormitorios y el
  ...
  \end{enumerate}
\item
  (1.0 puntos) Adicional a lo ya expuesto, se considera que la
...
\item
  (1.0 puntos) Considere ahora que las expectativas de variación de
...
\item
  (1.0 puntos) En todos los modelos anteriores se ha usado como
...
\item
  (1 punto) Considerando que los precios pueden variar año a año,
  ...
  últimos 3 años.
\item
  (1 punto) Disconformes con la capacidad de pronóstico de los modelos
...
\[\text{Tabla: Muestra de datos de 3 propiedades de arriendo en Chile, 2025}\]
...
\tightlist
\item
  (1 punto) Indexemos cada precio como \(P_{ijkt}\) representando la
...
\tightlist
\item
  (1 punto) Tenemos que redefinir los índices \(\beta_1\) y \(\beta_4\)
...
\setcounter{enumi}{2}
\item
  (1 punto) Para poder ingresar al modelo tenemos que calcular el
...
\item
  (1 punto) Hay al menos dos consideraciones a tener en cuenta, aunque
  ...
  \end{itemize}
\item
  (1 punto) Por definición \(ln P_{kt} = \theta' \bar{x}_{kt}\), en que
...
\item
  (1 punto) La clave de la pregunta es notar que las variables
...
\hypertarget{problema-5---p1-control-otouxf1o-2025}{%
\subsection{Problema 5 - P1 Control Otoño 2025}\label{problema-5---p1-control-otouxf1o-2025}}
...
\href{https://impresee.com/}{Impresee} y \href{https://visidea.ai/}{Visidea}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Plantee un modelo de regresión para describir el número de clics del
  usuario \emph{i} en el producto \emph{j} en la instancia de búsqueda \emph{k}, que
  permita entender las diferencias dependiendo de si la búsqueda se
  realiza usando imágenes o texto. Para eso considere que:

  \begin{itemize}
  \item
    El \emph{ranking} o posición en el listado de búsqueda afecta en los
    ...
    que se degradan los clics con el \emph{ranking} es distinta
  ...
  \end{itemize}
\item
  A partir de sus resultados de regresión, ¿Cómo determinaría
  ...
  búsqueda por texto?
\item
  Considere que los modelos de regresión confirman que, aunque haya un
  ...
  búsqueda \emph{k} del usuario \emph{i}, se puede calcular \(NNextSearch_{ik}\),
  ...
  \emph{k}. Para su modelo de regresión considere que, junto con el tipo de
  ...
  \item
    ...
    ejemplo, la categoría \emph{juguetes} en general tiene más búsquedas
    que la categoría \emph{colchones}.
  \item
    El número de resultados reportados en la búsqueda \emph{NumResult} y
    el número de categorías distintas reportadas en la búsqueda
    \emph{NumProdCat}. Considere que tanto \emph{NumResult} como \emph{NumProdCat}
    ya están calculados en la base de trabajo.
  \item
    El número de búsquedas diarias que en promedio hace el cliente
    \emph{AvgNumSearch} (que puede suponer conocido).
  \end{itemize}
\item
  Suponga que, después de un par de iteraciones, se ha determinado que
  ...
  \def\labelenumii{\alph{enumii})}
  \item
    ¿Cómo interpreta los resultados de la variable \emph{NumResult} en el
    contexto de esta aplicación?\\
  \item
    ¿Cómo interpreta el dramático aumento en el valor de \(R^2_{adj}\)
    ...
    \begin{longtable}[]{@{}ccc@{}}
    \toprule
    \textbf{Coefficient} & \textbf{(1)} & \textbf{(2)} \\
    \midrule
    \endhead
    ...
    \[\text{Tabla: Resultados de modelos de regresión próximas búsquedas}\]
  \end{enumerate}
\end{enumerate}
...
\item
  ...
  \]
\item
  Se puede utilizar un test F para evaluar que el grupo de variables
  de \(Image\), \(Ranking\) y \((1-Image)\) sean distintos en su conjunto.
\item
  Si llamamos a \(CAT_{ihk}\) a las dummy que toman el valor 1 si la
...
\item
  ...
  \tightlist
  \item
    Los coeficientes son significativamente negativos en ambas
  ...
  \item
    La diferencia entre las especificaciones (1) y (2) es que la
...
\hypertarget{problema-6---p1-control-primavera-2024}{%
\subsection{Problema 6 - P1 Control Primavera 2024}\label{problema-6---p1-control-primavera-2024}}
...
\emph{Whileus} es un conglomerado que tiene la representación de varias
...
de las marcas de \emph{Whileus}. El propósito del acuerdo es evaluar, en el
...
las tiendas de \emph{Whileus}.

En la primera semana del piloto y después de una reunión técnica entre
los equipos de \emph{Whileus} con la empresa de telecomunicaciones se acuerda
...
\def\labelenumi{\arabic{enumi}.}
\item
  Comente las ventajas teóricas de cada una de las formulaciones. Si
...
\item
  En la Tabla 1 se despliegan los resultados de un primer ejercicio de
  ...
  \def\labelenumii{\alph{enumii}.}
  \item
    ¿Cómo es posible que, usando los mismos datos, las ecuaciones 1
    y 2 nos reporten distintos estimadores del efecto de la
    distancia en la demanda de cada zona censal por cada marca?
  \item
    ¿Qué información necesita para cnstruir un intervalo de
  ...
  \item
    Si el objetivo del análisis sería tener el pronóstico más
    preciso de la demanda en cada tienda, ¿cómo elegiría entre estos
    dos modelos?
  \item
    ¿Cómo testearía si hay diferencias significativas en la demanda
    por marca?

    \[\text{Tabla: Resultados modelos alternativos de regresión}\]
  ...
  \end{enumerate}
\item
  Para evaluar la capacidad de los modelos para apoyar la gestión del
  ...
  \[\text{Tabla: Pronóstico de demanda para cuatro tiendas seleccionadas}\]
  ...
  \midrule
  \endhead
  & \textbf{Pesi mista} & \textbf{N eutro} & \textbf{Opti mista} & \textbf{Pesi mista} & \textbf{N eutro} & \textbf{Opti mista} \\
  ...
  errores estándar de pronóstico (\emph{sef}). Explique por qué los \emph{sef}
  son en general menores en los escenarios neutros.
\item
  Habiendo revisado la primera ronda de resultados, el equipo de
  ...
  \item
    ...
    cualquiera de las marcas de \emph{Whileus} al momento de hacer el
...
\def\labelenumi{\arabic{enumi}.}
\item
  La diferencia principal es que uno describe el efecto de
...
\item
  ...
  \tightlist
  \item
    Los estimadores dan cuenta del valor esperado de la variable
    dependiente \textbf{condicional} en la realización de las otras
    variables. Cada vez que cambiamos el conjunto de las variables
    sobre las que se condiciona, es esperable que cambien los
    estimadores.
  \item
    Necesitamos (una estimación de) los errores estándares. Como no
    están reportados en la tabla, no podemos calcular su valor
    numérico.
  \item
    Si el objetivo es pronóstico, habría que calibrar el modelo en
    ...
    como MAE o MAPE.
  \item
    Por medio de una prueba F. Por ejemplo, si queremos evaluar si
    hay diferencia entre las marcas A y C, podemos testear la
    hipótesis lineal \(\beta_A - \beta_C = 0\).
  \end{enumerate}
\item
  Los errores estándares de pronóstico dependen de las variables
...
\item
  ...
  \end{itemize}
\item
  Aunque hay otras ideas admisibles, el fraseo de la pregunta parece
  ...
  \tightlist
  \item
    Un nivel mayor de sofisticación podría sugerir usar
    \(Fracusuarios_{it-1}\) y/o \(Fracusuarios_{it-2}\). La inclusión de
    rezagos presupone que la adopción antecede las compras.
  \item
    Esta especificación puede ayudar a aminorar los problemas de
...
\hypertarget{problema-7---p1-control-1-otouxf1o-2024}{%
\subsection{Problema 7 - P1 Control 1 Otoño 2024}\label{problema-7---p1-control-1-otouxf1o-2024}}
...
puede hacer una etapa liviana de entrenamiento o de \emph{fine tuning}³.
...
\tightlist
\item
  \textbf{Prompt 1}: Se instruye al agente con una lista breve y resumida
  de cómo debe reaccionar a los requerimientos de los usuarios.
\item
  \textbf{Prompt 2}: Se instruye al agente con una lista detallada de cómo
  debe reaccionar a los requerimientos de los usuarios.
\item
  \textbf{Prompt 3}: Se instruye al agente con una lista detallada de cómo
...
\tightlist
\item
  \textbf{customer\_id}: Identificador del participante dentro del
  experimento.
\item
  \textbf{task\_id}: Identificador del escenario.
\item
  \textbf{age}: Edad del participante.
\item
  \textbf{sex}: Sexo del participante.
\item
  \textbf{NSE}: Nivel socioeconómico del participante.
\item
  \textbf{prompt\_type}: Tipo de prompt asignado en el experimento.
\item
  \textbf{resolutivity}: Resolutividad percibida por el participante.
\item
  \textbf{confidence}: Confianza percibida por el participante en la
  correctitud de la información.
\item
  \textbf{experience}: Satisfacción percibida por el participante.
\item
  \textbf{efficacy\_input}: Eficacia de comprensión de mensajes percibida
  por el participante.
\item
  \textbf{efficacy\_output}: Eficacia de generación de respuestas percibida
  por el participante.
\item
  \textbf{interaction\_time}: Duración total de la interacción del
  participante con el agente.
\item
  \textbf{cus\_res\_time}: Tiempo promedio que tarda en responder el
  participante en la interacción.
\item
  \textbf{agt\_res\_time}: Tiempo promedio que tarda en responder el agente
  en la interacción.
\item
  \textbf{n\_messages}: Número de mensajes en la interacción.
\item
  \textbf{n\_words}: Número de palabras promedio por mensaje.
\end{itemize}
...
\def\labelenumi{\arabic{enumi}.}
\item
  (1 punto) La compañía está considerando usar un(os) modelo(s) de
  ...
  de regresión?
\item
  (2 puntos) Sea \(y_{ik}\) la variable de desempeño que usted considera
  relevante para el participante i (\(i \in \{1,\ldots,643\}\)) en el escenario k (\(k \in \{1,2,3\}\)). Formule un
...
¹ \url{https://aws.amazon.com/es/what-is/generative-ai/}\\
² \url{https://aws.amazon.com/es/what-is/foundation-models/}\\
...
\def\labelenumi{\arabic{enumi})}
\item
  La respuesta más clara es la satisfacción de los usuarios
  ...
  \tightlist
  \item
    El agente debe seguir un conjunto de procedimientos definidos
  ...
  \item
    Uno de los motivos de la automatización es que el agente pueda
...
\item
  ...
  \tightlist
  \item
    La variable \(1_{ei}\) toma el valor 1 si el usuario i es asignado
  ...
  \item
    El efecto fijo \(\alpha_k\) controla por diferencias entre tipos
    de escenarios.
  \item
    El término \(\beta_{1}\) permite controlar que la evaluación
    dependa de la edad.
  \item
    Los términos \(\beta_{2}\) y \(\beta_{3}\) permiten capturar el
  ...
  \item
    El término \(\beta_4\) captura el efecto de ser hombre, sumando al
    efecto de \(\beta_5\) que captura la interacción entre el largo de
    la interacción y el sexo hombre.
  \end{enumerate}
\item
  Hay varias limitaciones, pero probablemente se puedan agrupar en las
  ...
  \tightlist
  \item
    Respecto al diseño de investigación. Por ejemplo, el diseño solo
    compara tres estrategias específicas, pero no sabemos cuál sería
    el desempeño de una nueva estrategia de entrenamiento.
  \item
    Respecto al modelo de regresión. Por ejemplo, hay factores que
...
\item
  ...
  \tightlist
  \item
    Los coeficientes del Prompt 2 es significativo en las tres
  ...
  \item
    Habría que testear si \(\theta_2 = \theta_3\) (por ejemplo, si
...
\textbf{Nota:} Para los curiosos, los p-valores de los test para las tres
...
\hypertarget{problema-8---p1-control-primavera-2023}{%
\subsection{Problema 8 - P1 Control Primavera 2023}\label{problema-8---p1-control-primavera-2023}}

Con el desarrollo de los canales digitales, muchas industrias han
\emph{desintermediarizado} la distribución de productos y servicios. Este es
precisamente el caso de la industria musical en que tanto artistas
consagrados\href{https://open.spotify.com/intl-es/artist/06Hl4z0CvFAxyc27GXpf02}{¹}
como
independientes\href{https://open.spotify.com/intl-es/artist/5eFvIKbhmaxjVEfsOCros}{²}
...
\tightlist
\item
  \textbf{IdSong}: Identificador único para cada canción.
\item
  \textbf{Date}: Fecha del registro.
\item
  \textbf{Title}: Título de la canción.
\item
  \textbf{Artist}: Intérprete de la canción.
\item
  \textbf{Genre}: Género musical al que pertenece la canción (e.g.~Rock,
  Pop, Indie, etc.)
\item
  \textbf{Length}: Duración de la canción (medido en segundos).
\item
  \textbf{LaunchDate}: Fecha en la que fue lanzada la canción.
\item
  \textbf{Nplay}: Número de veces que ha sido escuchada la canción.
\item
  \textbf{Tprom}: Tiempo de escucha promedio cada vez que es escuchada una
  canción.
\item
  \textbf{Danceability}: Puntaje {[}0-1{]} de qué tan bailable es una canción
  (basada en tiempo y regularidad).
\item
  \textbf{Energy}: Puntaje {[}0-1{]} de qué tan energética es una canción
  (basada en timbre y volumen).
\item
  \textbf{PromoExpend}: Monto del gasto en promoción dentro de la
  plataforma para esa fecha.
\item
  \textbf{Nfollow}: Seguidores en redes sociales de artista.
\item
  \textbf{Top50}: 1 si el artista ha tenido alguna vez una canción en el
...
\item
  ...
  \textbf{Observación:} Notar que la base está indexada por la canción y el
...
\item
  ...
  \tightlist
  \item
    Es importante controlar por la popularidad que típicamente tiene
    cada artista y que las canciones lanzadas más recientemente
    tienen acceso a un mayor número de usuarios activos en la
    plataforma.
  \item
    Las promociones aumentan la popularidad de las canciones, pero
    este efecto es acumulativo en el tiempo.
  \item
    El efecto de la \emph{danceability} de una canción puede ser positivo
    para unos géneros, pero negativo para otros.
  \item
    El efecto del largo de una canción es no lineal en la
...
\item
  Suponga que se estiman tres modelos simples de regresión lineal para
  describir la popularidad de las canciones dentro de la plataforma.
  Los resultados de estos dos modelos se reportan en la siguiente
  tabla
  ...
  \endhead
  \(\ln(\
  text{le ngth})\) & -0.453 & 0.001 & -0.462 & 0.023 & -0.3983 & 0.024 \\
  ...
  \item
    ¿Cuál es el intervalo de confianza del 95\% para el coeficiente
    \(NFollow\) en el modelo 2?
  \item
    ¿Qué puede explicar el cambio de significancia de \(Energy\) y
    \(Danceability\) en Modelos 1 y 2?
  \item
    ¿Cómo se puede testear que \(Danceability\) y \(Energy\), de manera
...
\item
  ...
  \item
    ...
    Hay varias otras posibilidades. Por ejemplo, usar el \emph{share} de
  ...
  \item
    ...
    el \emph{envolvimiento o lealtad} con la canción . Una canción podría
...
\item
  ...
  \tightlist
  \item
    Al comparar el Modelo 1 con el Modelo 2, vemos que el género
  ...
  \item
    ...
    \]
  \item
    En ambos casos una de las variables es significativa, pero la
  ...
  \item
    Queremos testear que \(\beta_{dance} = 0\) y \(\beta_{Energy} = 0\).
    Este es un sistema de restricciones lineales que puede testearse
    directamente a través de una prueba F.
  \end{enumerate}
\end{enumerate}

\hypertarget{problema-9---p1-control-otouxf1o-2023}{%
\subsection{Problema 9 - P1 Control Otoño 2023}\label{problema-9---p1-control-otouxf1o-2023}}

\emph{KornerK} introdujo hace un par de años un modelo de negocio innovador
...
los numerosos \emph{shoppers} que tiene afiliados, los que se encargan de
recoger los productos desde la sala de supermercado y transportarlo
hasta el domicilio del cliente. La asignación de \emph{shoppers} se hace
...
de esta coyuntura, \emph{KornerK} se ha visto obligado a rediseñar su
dotación y ha considerado hacer una reducción de hasta un 20\% de los
\emph{shoppers} en aquellas zonas con mayor caída en la demanda. Con este
...
opera \emph{KornerK}. En esta base, cada fila se compone de las siguientes
variables:

\begin{itemize}
\tightlist
\item
  \textbf{IdZip}: Identificador de la zona geográfica.
\item
  \textbf{Date}: Fecha en formato dd/mm/yyyy.
\item
  \textbf{Hour}: Un entero que representa la hora que define el bloque
  horario (8, 9, 10, \ldots, 22).
\item
  \textbf{NOrdersC}: Número de órdenes por servicio que fueron exitosamente
  completadas.
\item
  \textbf{NOrdersN}: Número de órdenes por servicio que no pudieron ser
  completadas por falta de \emph{shoppers}.
\item
  \textbf{Day}: Día de la semana que se registra la discrepancia (con
  lunes=1, hasta domingo=7).
\item
  \textbf{Mes}: Correlativo del número de meses desde inicio de operación
  (1, 2, 3, \ldots).
\item
  \textbf{Rain}: 1 si llovió en el bloque horario.
\item
  \textbf{IMACEC}: Variación del índice mensual de actividad económica.
\end{itemize}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Describa un modelo de regresión lineal para estimar la \textbf{demanda
  total} por servicios en cada zona y bloque horario que considere
  \textbf{al menos cuatro} de los siguientes elementos:

  \begin{enumerate}
  \def\labelenumii{\alph{enumii})}
  \item
    Efectos fijos geográficos para capturar que gran parte de la
    variabilidad de la demanda se explica por diferencias en los
    patrones de consumo de las distintas zonas.
  \item
    El patrón de demanda a lo largo de día es altamente estacional,
    por lo que es esperable que la inclusión de efectos fijos por
    hora sea relevante.
  \item
    Aunque hay alguna variación de la demanda dependiendo del día de
    semana, es despreciable en relación con la diferencia que hay
    entre días de semana y fin de semana, la que es muy relevante.
  \item
    El nivel de actividad económica parece estar positivamente
  ...
  \item
    La demanda ha ido creciendo sostenidamente a lo largo del
    ...
    tiempo.
  \item
    La demanda tiende a crecer en los bloques en que llueve. Sin
  ...
  \end{enumerate}
\item
  Suponga que se estiman dos modelos simples de regresión lineal para
  describir la demanda por servicios de \emph{KornerK}. Los resultados de
  ...
  \item
    ¿Qué podemos afirmar con respecto al rol de los efectos fijos
    regionales para describir la variabilidad en la demanda?
  \item
    ¿Qué rol juega la actividad económica en la demanda por
    servicios de \emph{KornerK}?
  \end{enumerate}
\item
  Volviendo al problema de gestión al que se enfrenta la compañía,
  \emph{KornerK} está interesado en evaluar cómo podría impactar la
  reducción de \emph{shoppers} en la capacidad de responder a la demanda.
  Para esto, ha elaborado un plan preliminar en que quedarían
  \(N_{ith}\) \emph{shoppers} disponibles para trabajar en el bloque horario
  \(h\) del día \(t\) en la zona \(i\).

  \begin{enumerate}
  \def\labelenumii{\alph{enumii})}
  \item
    Usando el modelo 1, calcule el valor esperado de la demanda para
    un bloque en un día de semana, en que no llueve y en que hay una
    variabilidad en la actividad económica del 10\% (\(IMACEC = 0.1\)).
  \item
    Para el mismo bloque horario de la parte anterior, escriba una
...
\def\labelenumi{\arabic{enumi}.}
\item
  Primero que todo, tenemos que clarificar que la variable dependiente
...
\item
  ...
  \def\labelenumii{\alph{enumii}.}
  \item
    Los modelos 2 y 3 tienen efectos fijos y, por tanto, hay un
  ...
  \item
    El \(R^2\) captura la varianza explicada. En este caso el modelo
    con mayor R2 es el modelo 3.
  \item
    Al agregar efectos fijos por región, el \(R^2\) (varianza
  ...
  \item
    En los tres modelos es positiva y significativa p-valor \textless{} 0.05,
...
\hypertarget{modelos-probabilisticos}{%
\chapter{Modelos Probabilisticos}\label{modelos-probabilisticos}}
...
\begin{itemize}
\item
  \emph{Duración}: La pregunta clave es ¿Cuándo? Son situaciones ligadas a
  ...
  continuo, o discreto.
\item
  \emph{Conteo}: La pregunta clave es ¿Cuántos? Son situaciones ligadas al
...
\item
  \emph{Elección}: La pregunta clave es ¿Cuál? Son situaciones asociadas a
...
\item
  ...
  requerida.
\item
  Identificar el comportamiento observable (heterogeneidad) de interés
...
\item
  ...
  \quad \label{eq:caso-continuo}
  \]

  \[
  p(x) = \sum_{i} f(x|\theta)\, Pr(\theta = \theta_{i}) 
  \quad \label{eq:caso-discreto}
  \]
\end{enumerate}
...
\hypertarget{problemas-teuxf3ricos-1}{%
\section{Problemas Teóricos}\label{problemas-teuxf3ricos-1}}

\hypertarget{e1-tasa-de-respuesta}{%
\subsection{E1: Tasa de respuesta}\label{e1-tasa-de-respuesta}}
...
\tightlist
\item
  Muy probablemente esté subestimando la tasa de respuesta de ese
  segmento.
\item
  Debiera redefinir los criterios de segmentación para hacer grupos
  más grandes y accionables.
\item
  Ha definido una política que da cuenta de su aversión al riesgo.
\item
  Debiera usar un modelo de duración en tiempo continuo con
  dependencia en la duración.
\item
  Debiera incorporar variables explicativas en su modelo predictivo.
\end{enumerate}

\hypertarget{e2-distribuciuxf3n-weibull}{%
\subsection{E2: Distribución Weibull}\label{e2-distribuciuxf3n-weibull}}
...
\hypertarget{e3-distribuciuxf3n-gamma-y-heterogeneidad}{%
\subsection{E3: Distribución Gamma y Heterogeneidad}\label{e3-distribuciuxf3n-gamma-y-heterogeneidad}}
...
\tightlist
\item
  Consistencia con el dominio de la probabilidad de abandono en cada
  período.
\item
...
\item
  Para generar una fórmula cerrada que pueda ser calculada de manera
  computacionalmente eficiente.
\end{enumerate}

\hypertarget{e4-modelos-de-duraciuxf3n}{%
\subsection{E4: Modelos de Duración}\label{e4-modelos-de-duraciuxf3n}}
...
\hypertarget{e5-modelo-nbd}{%
\subsection{E5: Modelo NBD}\label{e5-modelo-nbd}}

Un modelo NBD describe la demanda de botellas de \emph{oporto} en los últimos
...
\tightlist
\item
  Hay que hacer un modelo de regresión que considere la dinámica del
  problema.
\item
  El cálculo no se puede hacer directamente, sin embargo podemos
  recalibrar el modelo considerando un horizonte de un mes.
\item
  El histograma del número de botellas consumidas por cliente se
  moverá a la izquierda.
\item
  La probabilidad de comprar \(x\) botellas resulta ser simplemente 1/6
  veces las probabilidades calculadas para el primer semestre.
\item
  Ninguna de las anteriores.
\end{enumerate}

\hypertarget{e6-modelos-de-conteo}{%
\subsection{E6: Modelos de Conteo}\label{e6-modelos-de-conteo}}

Un analista propone el uso de modelos de conteo para describir la
intensidad de la participación de los usuarios de la red social \emph{X}. Los
...
\tightlist
\item
  Se debe agregar la data para considerar la actividad agregada en
  todo el horizonte.
\item
  Probablemente el comportamiento de los usuarios en el quinto día
  esté afectado por factores que no están presentes en los primeros
  cuatro días.
\item
  Hay que calcular esperanzas condicionales.
\item
  Los supuestos de comportamiento son errados y hay que desechar el
  modelo.
\item
  Todas las anteriores.
\end{enumerate}

\hypertarget{e7-tiempo-discreto-en-modelos-de-duraciuxf3n}{%
\subsection{E7: Tiempo discreto en Modelos de Duración}\label{e7-tiempo-discreto-en-modelos-de-duraciuxf3n}}
...
\textbf{E7: Tiempo discreto en Modelos de Duración}

\begin{itemize}
\item
  \textbf{Correcto.} Esto muestra que en fenómenos con tiempos muy
  dispersos, el modelo discreto se vuelve poco práctico, mientras que
  el continuo es más parsimonioso.
\item
  \textbf{Correcto.} En continuo se pueden modelar directamente con
  funciones de riesgo (hazard) dependientes del tiempo, lo cual puede
  ser más complejo de representar en discreto.
\end{itemize}

\hypertarget{problemas-aplicados-1}{%
\section{Problemas Aplicados}\label{problemas-aplicados-1}}

\hypertarget{problema-1-control-2-2025-2}{%
\subsection{Problema 1 (Control 2 2025-2)}\label{problema-1-control-2-2025-2}}
...
Recientemente, VTV ha lanzado un nuevo servicio llamado \emph{TuBienestar}.
...
programa \emph{TuBienestar}. Para eso, haremos uso de la base histórica de
...
\tightlist
\item
  (2.0 puntos) Actualmente, VTV describe el número de eventos de cada
  ...
  con ceros inflados. \emph{\textbf{Hint:} Le puede resultar útil conceptualizar
  el modelo pensando que con probabilidad} \(\pi\) \emph{no hay ningún evento
  y que con probabilidad} \(1-\pi\) \emph{el proceso sigue distribución de
  Poisson standard.}
\item
  ...
  \tightlist
  \item
    (1.0 puntos) Suponiendo que puede haber segmentos con mayor
    propensión a registrar eventos, escriba la log-verosimilitud de
    un modelo binomial con dos clases latentes.
  \item
    (1.0 puntos) Suponga que ha estimado el modelo y los estimadores
...
\setcounter{enumi}{2}
\item
  Como último enfoque, se estiman tres versiones de modelos
  ...
  \textbf{N. Obs} & 34.825 & 34.825 & 34.825 \\
  \textbf{LL} & -51.771,8 & -45.282,1 & -41.321,9 \\
  \textbf{AIC} & 107.543,6 & 90.580,2 & 82.665,8 \\
  \bottomrule
  \end{longtable}
  ...
  \tightlist
  \item
    (1.0 puntos) Discuta si el programa \emph{TuBienestar} ayuda a
    describir el comportamiento de los beneficiarios y si el
    programa \emph{TuBienestar} es bueno para la salud de los
    beneficiarios.
  \item
    (1.0 puntos) En otros mercados el valor esperado de la
...
\def\labelenumi{\arabic{enumi}.}
\item
  Siguiendo la indicación, la probabilidad de los eventos viene dada
  por:
  ...
  \]
\item
  Sean \(\theta_1\) y \(\theta_2\) las probabilidades de registrar un
  evento de las clases 1 y 2 respectivamente.

  \begin{enumerate}
  \def\labelenumii{\alph{enumii}.}
  \item
    La probabilidad de observar exactamente \(n\) eventos en el
    segmento de clientes \(j\) de la empresa \(k\) en el trimestre \(t\)
    viene dada por:

    \[ \Pr(n_{jkt} = n \mid \theta_1, \theta_2, \pi) = \pi \binom{m}{n_{jkt}} \theta_1^{n_{jkt}} (1 - \theta_1)^{m - n_{jkt}} + (1 - \pi) \binom{m}{n_{jkt}} \theta_2^{n_{jkt}} (1 - \theta_2)^{m - n_{jkt}} \]Entonces,
  ...
  \end{enumerate}
\item
  La parte a) requiere mirar los resultados de la tabla. La parte b)
  puede contestarse independientemente.

  \begin{enumerate}
  \def\labelenumii{\alph{enumii}.}
  \item
    El AIC del modelo 2 es mayor que el AIC del modelo 3 que solo
  ...
  \item
    Recordar que la esperanza de una \(Beta(\alpha,\beta)\) viene dada
...
\def\labelenumi{\arabic{enumi}.}
\item
  Proponga un modelo de conteo (debe definir la variable dependiente
  ...
  \tightlist
  \item
    La industria y la zona a la que pertenecen cada cuenta incide en
    la posibilidad de generar y convertir más o menos oportunidades.
  \item
    El tiempo de relación afecta las oportunidades ganadas. El
    ...
    año.
  \item
    Se espera que la participación en eventos presenciales y
  ...
  \item
    El número total de interacciones de los empleados debiera
  ...
  \item
    Los procesos de conversión son relativamente lentos, por lo que
  ...
  \end{enumerate}
\item
  Usando los mismos supuestos del modelo anterior, construya un modelo
  ...
  \emph{\textbf{Hint:} Si le resulta conveniente en su notación, puede asumir
  que la tasa del modelo de conteo que captura todas las componentes
  relevantes del problema se denota} \(\lambda_{it}(x_{it})\)\emph{, con}
  \(x_{it}\) \emph{un conjunto de variables observables de la cuenta} \(i\) \emph{en
  el año} \(t\)\emph{.}
\item
  Considerando que, quizás, la heterogeneidad observable no es
  ...
  \begin{longtable}[]{@{}ll@{}}
  \caption{Log-verosimilitud modelos alternativos. \{\#tbl:loglik\}}\tabularnewline
  \toprule
...
\item
  ...
  \def\labelenumii{\alph{enumii}.}
  \item
    Necesitamos incluir dummies para zonas (\(\text{Zone}_{ih_1}\)) y
  ...
  \item
    Se pueden generar dos variables para los niveles de antigüedad
  ...
  \item
    Hay que incluir el número de eventos físicos, digitales y su
    interacción.
  \item
    El número total de interacciones es la multiplicación del número
  ...
  \end{enumerate}
\item
  El modelo es idéntico al anterior, pero en vez de describir la
  ...
  Aquí, \(m_{it}\) es el número de \emph{leads} generados por la cuenta en el
...
\item
  Para preferir el modelo beta binomial, el AIC debiera ser menor que
...
programa de capacitación que ha denominado \emph{El Club del Almacenero}, que
...
\def\labelenumi{\arabic{enumi})}
\item
  Suponga que el número de niveles de los cuales participa un
  ...
  \tightlist
  \item
    (1 punto) Calcule una expresión para la probabilidad que un
  ...
  \item
    (1 punto) Escriba la log-verosimilitud del problema. Al igual
    que en el caso anterior, exprese la log-verosimilitud en
    términos de \(x_{ikt}\), \(\alpha\) y \(\beta\).
  \end{enumerate}
\item
  Suponga que se estima el modelo usando el método de la máxima
  verosimilitud resultando los valores de la Tabla:

  \begin{longtable}[]{@{}rr@{}}
  \caption{\textbf{Tabla:} Resumen de ventas para cada función.}\tabularnewline
  \toprule
  ...
  \def\labelenumii{\alph{enumii})}
  \item
    La Figura reporta la distribución beta evaluada en los
    ...
    \begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth,height=\textheight]{images/fig_beta_posterior_v2.png}
    \caption{\textbf{Figura:} Distribución posterior de la probabilidad de no continuar}
    \end{figure}
  \item
    Calcule el intervalo de confianza al 95\% del segundo parámetro
  ...
  \end{enumerate}
\item
  Suponga ahora que se rediseña el programa de modo que los contenidos
  ...
  beta-binomial.
\item
  Si los estimadores máximos verosímiles de la beta binomial son
...
\def\labelenumi{\arabic{enumi})}
\item
  Sea \(D_{it}\) el número de niveles cursado por el almacenero \(i\) en
...
\item
  ...
  \def\labelenumii{\alph{enumii})}
  \item
    La probabilidad \(p\) corresponde al área bajo la curva, que puede
    aproximarse como\\
    \[
    ...
    \begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth,height=\textheight]{images/fig_beta_posterior_shaded_08_1.png}
    \caption{\textbf{Figura:} Distribución posterior con área sombreada}
    \end{figure}
    ...
    \tightlist
    \item
      Usando el inverso del hessiano, el ientervalo de confianza
...
\item
  ...
  \]
\item
  La probabilidad solicitada es simplemente la esperanza condicional
...
En 1972, la \emph{National Association of Food Chains} (NAFC) impulsó el uso
...
\tightlist
\item
  \textbf{Id}: Identificador de un evento de discrepancia de precios.
\item
  \textbf{Day}: Día de la semana que se registra la discrepancia (con lunes
  = 1 y domingo = 7).
\item
  \textbf{Type}: Variable categórica con el tipo de discrepancia (Tipo I o
  Tipo II).
\item
  \textbf{Section}: Categoría del supermercado a la que pertenece el
  producto que presenta un diferencial (Abarrotes, Limpieza, Bebidas,
  Galletas y Cocktail).
\item
  \textbf{Duration}: Número de días que dura la discrepancia de precios
  entre el fleje y el sistema transaccional.
\item
  \textbf{Tag\_price}: Precio que aparece en el fleje.
\item
  \textbf{Real\_price}: Precio cobrado en caja.
\item
  \textbf{diff}: Diferencia entre precio fleje y precio real.
\end{itemize}

Para ilustrar la estructura de la base, las primeras entradas se
presentan en la Tabla 1:

\begin{longtable}[]{@{}rrclrrrr@{}}
\toprule
...
\def\labelenumi{\arabic{enumi}.}
\item
  Describa un modelo de duración probabilístico que permita describir
...
\item
  Suponga que complementario a los modelos de duración, se estiman dos
  ...
  \textbf{R²} & \textbf{0.096} & & \textbf{0.136} & \\
  \textbf{Adj. R²} & \textbf{0.086} & & \textbf{0.120} & \\
  \bottomrule
  \end{longtable}
...
\item
  Suponga que ahora se estiman tres modelos para describir la duración
  ...
  Diff & -- & -- & -0.43 \\
  limpieza & -- & -- & -0.87 \\
  \textbf{LL} & -788 & -761 & -566 \\
  \textbf{AIC} & 1278 & 1526 & 1142 \\
  \bottomrule
  \end{longtable}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Dado que la duración la observamos en días, parece más natural
...
\item
  Hay varias consideraciones que sugieren que el \textbf{Modelo 2} podría
  ser preferible.

  \begin{itemize}
  \tightlist
  \item
    \texttt{diff} no es significativo en modelo 1, pero \texttt{log(diff)} sí lo
    es en modelo 2.
  \item
    El p-valor del tipo es menor en el modelo 2.
  \item
    La interacción que agrega el modelo 1 (\texttt{type.I\ ×\ diff}) no es
    significativa.
  \item
    Las variables adicionales del modelo 2 sí son significativas.
  \end{itemize}
\item
  Las expresiones se derivan directamente de las definiciones y vienen
  dadas por:

  \begin{itemize}
  \item
    \textbf{Geométrica:} \(\Pr(T>3\mid \theta_0)=(1-0.33)^3\).
  \item
    \textbf{Beta-Geométrica:} \[
    \Pr(T>3\mid \alpha,\beta)\;=\;\frac{B(2.08,\;2.62+3)}{B(2.08,\;2.62)}.
    \]
  \item
    \textbf{Regresión Geométrica:} \[
...
estadía en la compañía. \emph{"¡Hola! Esperando que estés muy bien, te
...
sientas integrada te vamos a ir copiando en las} \emph{comunicaciones
...
Mes & Mes \{enero, febrero, \ldots, diciembre\} & - \\
Año & Año & - \\
...
\def\labelenumi{\arabic{enumi}.}
\item
  Proponga un modelo de regresión de Poisson en que la tasa de compra
  ...
  \item
    El género induce diferencias relevantes en las ventas y el
    efecto del género es distinto para la zona oriente de la
    capital.
  \item
    Existe una tendencia creciente en las ventas. Como el
    crecimiento es relativamente pequeño, puede modelarse con un
    efecto fijo a nivel de año.
  \item
    Aunque en el consumo es relativamente constante a lo largo del
  ...
  \end{enumerate}
\item
  La investigación exploratoria de los datos sugiere que,
...
\item
  Después de revisar los resultados de los modelos anteriores, te
...
\def\labelenumi{\arabic{enumi}.}
\item
  En un modelo de regresión de Poisson, la distribución ya viene dada
  ...
  Con esto, la \textbf{log-verosimilitud} viene dada por: \[
  LL
...
\item
  La utilidad que deriva el cliente \(i\) por comprar en la categoría
...
un creciente interés por convertirse en \emph{creador de contenido}, que les
...
(ver, comentar, compartir), la plataforma considera que el número de \emph{me
...
Nlikes & Número de \emph{me gusta} que ha recibido la publicación & 12.5 \\
\bottomrule
\end{longtable}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Modele el número de \emph{me gusta} que recibe una publicación como un
  ...
  \tightlist
  \item
    Justifique la inclusión de al menos dos variables explicativas
    indicando cuál es el signo esperado del coeficiente
    correspondiente.
  \item
    Escriba la log-verosimilitud del problema. Indique cuáles son
    los parámetros para estimar y explicite cuántos parámetros tiene
    su modelo.
  \item
    Suponiendo que ha estimado el modelo y los parámetros proveen
    una buena descripción de la distribución del número de \emph{me
    ...
    menos un \emph{me gusta}.
  \end{enumerate}
\item
  Una de las limitaciones del modelo anterior es que el uso del
  ...
  \tightlist
  \item
    Describa el proceso como un proceso de elección binaria con
  ...
  \item
    Si al estimar el modelo, se encuentra que \(\alpha=1\) y \(\beta=2\)
    son los estimadores máximo-verosímiles. En promedio, ¿con qué
    probabilidad a un cliente le gusta una publicación en la
    plataforma?
  \item
    Considere una publicación que ha sido desplegada 100 veces y ha
    recibido 10 \emph{me gusta}. Considerando los valores de
    \((\alpha,\beta)=(1,2)\), de la parte anterior, ¿cuántos \emph{me
...
\item
  ...
  \def\labelenumii{\alph{enumii}.}
  \item
    Cualquiera de las variables listadas puede justificarse de
    manera más o menos directa:

    \begin{itemize}
    \item
      \textbf{Efecto fijo por creador:} hay creadores más populares que
      otros.
    \item
      \textbf{Efecto fijo por fecha:} hay fechas en que los usuarios
      son más activos.
    \item
      \textbf{Dummies por categoría:} hay categorías que pueden generar
      más \emph{me gusta}.
    \item
      \textbf{Nchar:} textos más largos o más cortos pueden ser más
      atractivos.
    \item
      \textbf{Npic:} agregar más fotos puede ser más atractivo.
    \item
      \textbf{Tvid:} videos más largos o más cortos pueden ser
      preferidos.
    \item
      \textbf{AIproduction:} material mejor producido puede generar más
      \emph{me gusta}.
    \item
      \textbf{AIcontent:} contenido de mejor calidad puede generar más
      \emph{me gusta}.
    \item
      \textbf{AInovelty:} material más novedoso puede generar más \emph{me
      gusta}.
    \end{itemize}
  \item
    Aunque cualquier combinación de las variables arriba descritas
    es admisible, para efectos ilustrativos consideraremos un modelo
    en que la tasa de \emph{me gusta} del contenido \(i\) viene dada por:
    \[
       ...
       \]
  \item
    La probabilidad de recibir al menos un \emph{me gusta} viene dada
    por: \[
  ...
  \end{enumerate}
\item
  El modelo es una beta-binomial estándar. Recordar que la esperanza
  ...
  \item
    ...
    Entonces, el número esperado de \emph{me gusta} al mostrar el
...
\def\labelenumi{\arabic{enumi}.}
\item
  Muestre que la penetración de mercado (fracción de clientes que
  compró al menos una unidad) puede expresarse como
  \(\frac{\beta}{\alpha+\beta}\).
\item
  Para ello considere que la penetración de mercado es del 60 \% y que
  ...
  modelo. \emph{Hint: Realice un sistema de ecuaciones.}
\item
  Se desea extender el modelo para que además incluya un segmento de
  ...
  la base de datos (i = 1, \ldots, n). Deduzca la función de
...
\item
  ...
  \]
\item
  Sea \(n_x\) el número de clientes que compra x parkas. Entonces, la
...
\tightlist
\item
  Use el método de los momentos para estimar un modelo binomial sin
  heterogeneidad que caracterice la probabilidad que un usuario
  intente atrapar a wigglytuff. \emph{\textbf{Hint:} Con usar el método de los
  ...
  binomial es} \(np\)\emph{.}
\item
  Escriba ahora un modelo que incorpore heterogeneidad para
  caracterizar la probabilidad que un número de usuarios intente
  atrapar a un pokemón \(k\) en una comuna \(c\).
\item
  Escriba la log-verosimilitud del modelo anterior, para entender la
...
\item
  Como debemos considerar heterogeneidad, ahora el parámetro de la
...
\setcounter{enumi}{2}
\item
  Nos interesa analizar cada personaje por separado, por lo tanto
...
\def\labelenumi{\arabic{enumi}.}
\item
  Plantee un modelo de elección para determinar la probabilidad de
  ...
  \item
    Sea \(N\) el número de clientes. Si se observa el comportamiento
...
\setcounter{enumi}{1}
\item
  Considere que se cuenta con información demográfica de los usuarios
  ...
  segmento canjeador \emph{C} y otro no-canjeador \emph{N}). \emph{\textbf{Hint}: Recuerde
  que si} \(x\in\mathbb{R}\)\emph{, entonces}
  \(\frac{exp(x)}{1+exp(x)}\in(0,1)\).
\item
  Suponga que se ha estimado el modelo anterior y se encuentra que los
...
\item
  Suponga ahora que del total de \(m_{i}\) mensajes que recibe un
...
\item
  ...
  \tightlist
  \item
    Sea \(X_i\) el número de veces que la persona \(i\) hace efectivo un
    ...
    \]
  \item
    Se deben estimar \(N\) parámetros, uno para cada cliente (lo que
    sugiere introducir heterogeneidad).
  \end{enumerate}
\item
  Modelo de elección con dos segmentos. Sea \(p_{iC}\) la probabilidad
...
\item
  Probabilidad posterior de pertenecer al segmento canjeador dado
  ...
  \]
\item
  Lectura y canje con dos etapas. La probabilidad de leer un mensaje
  es:
...
Los modelos de elección discreta buscan modelar \textbf{comportamiento} (toma de decisiones), describiendo la probabilidad de que un agente \(n\) elija la alternativa \(i\), es decir, \(P_{ni}\). Se asume que el individuo decide entre las alternativas tal que éste maximiza sus utilidades \(u_{ni}\).
...
\hypertarget{persistencia-de-elecciuxf3n-lealtad-y-precios-de-referencia}{%
\subsubsection{Persistencia de Elección (Lealtad) y Precios de Referencia}\label{persistencia-de-elecciuxf3n-lealtad-y-precios-de-referencia}}

En modelos de panel con observaciones repetidas, es posible incorporar efectos dinámicos como la \textbf{lealtad} y los \textbf{precios de referencia}:

\textbf{Lealtad:} Se introduce una variable de lealtad latente \(z_{nit}\) que captura la propensión del individuo a repetir elecciones previas:

\[z_{nit} = \lambda z_{nit-1} + (1-\lambda) y_{nit-1}\]

donde \(0 \leq \lambda \leq 1\) es un parámetro de decaimiento.

\textbf{Precios de Referencia:} Los consumidores comparan precios actuales con un precio de referencia interno formado a partir de precios observados en el pasado:
...
El modelo probit es un modelo de elección discreta utilizado para analizar decisiones en las que un individuo selecciona una alternativa entre un conjunto finito de opciones. Este modelo asume que los individuos maximizan su utilidad \(u_{ni}\), la cual se divide en una componente determinista (observable) \(v_{ni}\) y una estocástica (no observable) \(\varepsilon_{ni}\), con la componente estocástica siguiendo una \textbf{distribución normal estándar}.
...
Los parámetros del modelo se estiman usando el \textbf{método simulado de máxima verosimilitud (SMLE)}:

\[\hat{\theta} = \arg\max \sum_n \sum_i y_{ni} \ln(\hat{P}_{ni})\]

\hypertarget{modelo-nested-logit}{%
\subsection{Modelo Nested Logit}\label{modelo-nested-logit}}
...
\hypertarget{modelo-mixed-logit}{%
\subsection{Modelo Mixed Logit}\label{modelo-mixed-logit}}
...
\textbf{Estimación:} Para estimar los parámetros, se utiliza el \textbf{método de máxima verosimilitud simulada (SMLE)}. La probabilidad simulada se calcula como:
...
\hypertarget{logit-con-clases-latentes}{%
\subsection{Logit con Clases Latentes}\label{logit-con-clases-latentes}}
...
\textbf{Estimación:} Para estimar los parámetros, se maximiza la función de verosimilitud:
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Especificación de la utilidad:} Definir la función de utilidad \(u_{ni}\) con sus componentes determinísticas \(v_{ni}\) y estocásticas \(\varepsilon_{ni}\).
\item
  \textbf{Derivación de probabilidades:} Calcular las probabilidades de elección \(P_{ni}\) según el modelo especificado (Logit, Probit, Nested Logit, Mixed Logit, o Clases Latentes).
\item
  \textbf{Construcción de la verosimilitud:} Escribir la función de (log-)verosimilitud basada en las observaciones \(y_{ni}\).
\item
  \textbf{Estimación:} Para modelos con solución cerrada (Logit, Nested Logit, Clases Latentes), se utiliza \textbf{máxima verosimilitud directa}. Para modelos sin solución cerrada (Probit, Mixed Logit), se emplea \textbf{máxima verosimilitud simulada (SMLE)}.
\item
  \textbf{Evaluación:} Calcular métricas de ajuste como \(\rho\) de McFadden, AIC, BIC y validar la significancia estadística de los parámetros.
\item
  \textbf{Interpretación:} Analizar los parámetros estimados, calcular elasticidades y evaluar implicaciones para la toma de decisiones.
\end{enumerate}

\hypertarget{problemas-teuxf3ricos-2}{%
\section{Problemas Teóricos}\label{problemas-teuxf3ricos-2}}
...
\item
  Sea \(\pi\) la probabilidad de que un cliente pertenezca al segmento 1. Entonces, la probabilidad de elección viene dada por:

  \[P_{ni} = \pi \frac{\exp(v(\theta_1))}{1 + \exp(v(\theta_1))} + (1-\pi) \frac{\exp(v(\theta_2))}{1 + \exp(v(\theta_2))}\]
\item
  Definimos la componente determinística de la utilidad como:
...
\hypertarget{problemas-aplicados-2}{%
\section{Problemas Aplicados}\label{problemas-aplicados-2}}

\hypertarget{pregunta-1-1}{%
...
\begin{Shaded}
\begin{Highlighting}[]
\SpecialCharTok{\textgreater{}}\NormalTok{ mymle}\SpecialCharTok{$}\NormalTok{par}
\NormalTok{[}\DecValTok{1}\NormalTok{] }\FloatTok{0.124} \FloatTok{0.486} \SpecialCharTok{{-}}\FloatTok{2.351}
\SpecialCharTok{\textgreater{}}\NormalTok{ mymle}\SpecialCharTok{$}\NormalTok{value}
\NormalTok{[}\DecValTok{1}\NormalTok{] }\FloatTok{1344.35}
\end{Highlighting}
\end{Shaded}
...
\item
  ...
  \item
    Hay al menos dos formas de modelar este problema. La primera en que los interceptos se definen al nivel marca-formato y otra en que los interceptos se definen en términos de marca y formato:
...
\item
  ...
  {\centering \includegraphics[width=0.5\linewidth]{images/probit_bivariado} 

  }

  \caption{Plot de variables transformadas logarítmicamente.}\label{fig:probit-bivariado}
  \end{figure}
...
\tightlist
\item
  Suponga que para perfilar a los clientes se propone un modelo logit en que el usuario \(n\) deriva una utilidad \(u_n\) por darle un ``me gusta'' a alguna de las componentes de la campaña y 0 si no. Escriba la log-verosimilitud del problema asumiendo que la componente determinística de la utilidad depende linealmente de \(x_n = (Hombre_n, Twitter_n, Edad_n, Edad_n^2)\).
\end{enumerate}

Suponga que analistas dentro de la compañía han codificado la verosimilitud anterior en R en una función \texttt{loglikel} que recibe como primer argumento el intercepto de la función de utilidad y los cuatro siguientes los otros parámetros de la utilidad en el orden antes planteado. Para encontrar los estimadores máximo verosímiles se ha ejecutado el siguiente comando:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{mymle }\OtherTok{=} \FunctionTok{optim}\NormalTok{(}\AttributeTok{par=}\FloatTok{0.01}\SpecialCharTok{*}\FunctionTok{rep}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{5}\NormalTok{), }\AttributeTok{fn=}\NormalTok{loglikel, }\AttributeTok{hessian=}\ConstantTok{TRUE}\NormalTok{, }\AttributeTok{method=}\StringTok{"BFGS"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\SpecialCharTok{\textgreater{}} \FunctionTok{cbind}\NormalTok{(mymle}\SpecialCharTok{$}\NormalTok{par, }\FunctionTok{sqrt}\NormalTok{(}\FunctionTok{diag}\NormalTok{(}\FunctionTok{solve}\NormalTok{(mymle}\SpecialCharTok{$}\NormalTok{hessian))))}
\NormalTok{     [,}\DecValTok{1}\NormalTok{]      [,}\DecValTok{2}\NormalTok{]}
\NormalTok{[}\DecValTok{1}\NormalTok{,] }\SpecialCharTok{{-}}\FloatTok{0.9623}  \FloatTok{0.2069}
\NormalTok{[}\DecValTok{2}\NormalTok{,]  }\FloatTok{3.4340}  \FloatTok{0.0518}
\NormalTok{[}\DecValTok{3}\NormalTok{,]  }\FloatTok{0.0047}  \FloatTok{0.0522}
\NormalTok{[}\DecValTok{4}\NormalTok{,]  }\FloatTok{0.0365}  \FloatTok{0.0043}
\NormalTok{[}\DecValTok{5}\NormalTok{,] }\SpecialCharTok{{-}}\FloatTok{0.0048}  \FloatTok{0.0025}
\end{Highlighting}
\end{Shaded}
...
\setcounter{enumi}{1}
\item
  ¿Cuál es la probabilidad que una mujer de 20 años que no es usuaria de twitter le de un ``me gusta'' a la campaña?
\item
  ¿Cómo varía la probabilidad de elección anterior si la mujer sí es usuaria de twitter? Describa que implicancias podría tener su resultado en la elaboración de próximas campañas.
\end{enumerate}
...
\def\labelenumi{\alph{enumi})}
\item
  La componente determinística de la utilidad viene dada por:
...
\item
  Para esta mujer, la componente sistemática de la utilidad viene dada por:
...
\item
  Si fuere usuaria de twitter, basta modificar la componente determinística de la utilidad \(v_n \to v_n + 0.047 = -2.1476\) y recalcular la probabilidad:
...
\tightlist
\item
  Si las componentes \(\varepsilon_{1n}\), \(\varepsilon_{2n}\), \(\varepsilon_{3n}\), \(\varepsilon_{4n}\) son todos independientes, discuta brevemente si hay algún beneficio de estudiar simultáneamente los comportamientos de gasto y tenencia de tarjeta.
\end{enumerate}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Si \(\varepsilon_{3n}\) y \(\varepsilon_{4n}\) son independientes y distribuidos valor extremo, entonces la probabilidad que el cliente elija adquirir la tarjeta viene dada directamente por la fórmula del logit, donde la utilidad de no adquisición se ha normalizado en 0:
...
\def\labelenumi{\arabic{enumi}.}
\item
  Escriba una expresión para \(P_{EA}\), la probabilidad que un cliente espere sin comprar en la pre-venta y luego decida asistir al evento.
\item
...
\item
  Como analista, le interesa investigar las restricciones de comportamiento \(\alpha_1 = \alpha_3\) y \(\alpha_2 = 0\). Explique qué interpretación podría tener dichas hipótesis y cómo podrían testearse.
\end{enumerate}
...
\begin{itemize}
\item
  \textbf{Masivo multimedia:} en este paquete la agencia se compromete a alcanzar un cierto nivel de GRP para lo cuál puede hacer uso de cualquier medio. Los promedios históricos sugieren que un 40\% se gasta en TV, un 25\% en prensa escrita, un 20\% en radio y un 10\% en medios digitales.
\item
  \textbf{Nicho multimedia:} en este paquete la agencia se compromete a alcanzar un cierto nivel de GRP, pero concentrado en medios especializados. Así, dependiendo del rubro, la marca puede elegir entre algunas de las variantes temáticas disponible como deportes, infantil, outdoors, entre otros.
\item
  \textbf{Nicho radio y escrito:} Similar al paquete Nicho multimedia, pero restringiendo la parrilla a prensa escrita y a radio. A raíz de esta restricción el paquete conlleva un importante descuento en el precio.
\item
